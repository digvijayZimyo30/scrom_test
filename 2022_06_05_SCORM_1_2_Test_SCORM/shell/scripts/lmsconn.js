	...
	if(API_obj){
		switch(cmd){
			case "start": 
				...
				isexiting=0
				if(v_lmsmode=="1_2"){
					if(!API_obj.LMSInitialize) f_lmsaction("start")
					lresult=API_obj.LMSInitialize("")
				}else if(v_lmsmode=="1_3"){
					if(!API_obj.Initialize) f_lmsaction("start")
					...
					if(uri.indexOf("file:///")!=-1){
						nuri=nuri.split("?")[0]
						nuri=nuri.replace("file:///","")
						var pos=nuri.indexOf(":")
						if(pos==-1){
							nuri="http://"+nuri.substr(pos+1)
						}else{
							nuri="http:/"+nuri.substr(pos+1)
						}
						nuri+="?"+uri.split("?")[1]
					}
					xhr.open("GET","../tincan.xml",true);
					xhr.onreadystatechange = function() {
						...
						if (stat == 404) {
							xhr.abort();
							f_log("Unable to load file 'tincan.xml'",true,false);
						}
						if (stat == 200 || (v_clientinfo.islocal && stat == 0)) {
							if (xhr.readyState == 4) {
								var xml=_util.parseXML(xhr.responseText)
								xml=_util.xmlNode(xml, "activities.activity")
								datamodel.base = {
								    id: xml.getAttribute("id"),
								    definition: {
								    	type: "http://adlnet.gov/expapi/activities/course",
								        name: {
								            "en-US": _util.xmlNode(xml,"name").childNodes[0].nodeValue
								        },
								        description: {
								            "en-US": _util.xmlNode(xml,"description").childNodes[0].nodeValue
								        }
								    }
								};
								datamodel.config={
									endpoint:getpropdata("endpoint", qrstr),
									user:"",
									password:""
								}
								
								API_obj = new TinCan({url: nuri, activity: datamodel.base})
								if(API_obj.actor){
									datamodel.student_name=API_obj.actor.name
									datamodel.connected=true
								}else{
									datamodel.connected=false
								}
								if(typeof appBridge !== "undefined") datamodel.connected=false
								x_getState()
							}
						}
					};
...
function f_setIntData(i_obj){
	...
	if(v_lmsmode=="xAPI"){
		...
		f_setvalue(iref + ".result",i_obj.qres ? i_obj.qres : "")
		...
		//for (i=0; i < i_obj.cresp.length; i++){
			...
			f_setvalue(iref+".correct_responses.0.pattern",i_obj.cresp)
	    //}
	    if(v_lmsmode=="1_3"){
			//f_setvalue(iref + ".learner_response",i_obj.lresp.join("<OPTION>"))
			f_setvalue(iref + ".learner_response",i_obj.lresp)
		}else{
			//f_setvalue(iref + ".student_response",i_obj.lresp.join("<OPTION>"))
			f_setvalue(iref + ".student_response",i_obj.lresp)
		}
